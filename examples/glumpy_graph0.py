...
def on_timer(dt):
    ...
    X -= 0.5 * np.bincount(src, D[:, 0], minlength=len(N))
    Y -= 0.5 * np.bincount(src, D[:, 1], minlength=len(N))
    X += 0.5 * np.bincount(tgt, D[:, 0], minlength=len(N))
    Y += 0.5 * np.bincount(tgt, D[:, 1], minlength=len(N))
    ...
    # -----------------------------
    sel_color = np.array([1, 0.8, 0.2, 1]) - (dt / 60) * (
        np.array([1, 0.8, 0.2, 1]) - np.array([0.2, 0.2, 0.2, 0.2])
    )

    if selected > -1:
        XY[selected] = XY_[selected]
        markers["bg_color"][selected] = tuple(sel_color)  # (0.2, 0.2, 0.2, 0.2)
...
markers = collections.MarkerCollection(
    marker="disc", transform=transform, viewport=viewport
)
markers.append(
    N,
    size=15,
    ...
    itemsize=1,
    fg_color=(1, 1, 1, 1),
    bg_color=[(1, 0.8, 0.2, 1) for i in range(n)],
)
segments = collections.SegmentCollection("agg", transform=transform, viewport=viewport)
segments.append(
    N[src], N[tgt], linewidth=1.0, itemsize=1, color=(0.75, 0.75, 0.75, 1.00)
)
