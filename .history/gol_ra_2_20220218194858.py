...
void main(void)
{
    ...
    if( old_state > 0.5 ) {
        ...
        else if( count > 8.0 * params[1])
            new_state = 0.0;
    } else {
        // Any dead cell with exactly three live neighbours becomes
        //  a live cell, as if by reproduction.
       if( (count > 8.0 * params[2]) && (count < 8.0 * params[3]) )
           new_state = 1.0;
    }
    if( pingpong == 0 ) {
...
def on_character(character):
    ...
    if character == "A":
        params = np.random.rand(4)
        params = params.astype(np.float32) / 20 * np.random.randint(-1, 1)
    ...
    if character == "a":
        # params.sort()
        compute["params"][0] += MUTATION_STEP
    ...
    if character == "s":
        # params.sort()
        compute["params"][1] += MUTATION_STEP
    ...
    if character == "d":
        # params.sort()
        compute["params"][2] += MUTATION_STEP
    ...
    if character == "f":
        # params.sort()
        compute["params"][3] += MUTATION_STEP
    ...
    if character == "C":
        compute['params'] = np.array([0.1875, 0.4375, 0.3125, 0.4375], dtype=np.float32)        # print(f'Parameters mutated to: {compute["params"]}')
...
compute['dy'] = 1.0 / h
compute['pingpong'] = pingpong
compute['params'] = np.array([0.1875, 0.4375, 0.3125, 0.4375], dtype=np.float32)
