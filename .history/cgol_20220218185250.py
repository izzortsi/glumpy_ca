...
void main(void)
{
    ...
    if( old_state <= params[0] ) {
        // Any live cell with fewer than two live neighbours dies
        // as if caused by under-population.
        if( count  < 1.5*(1.0+old_state) )
            new_state = 0.0;
        ...
        else if( count > 3.5*(1.0+old_state) )
            new_state = 0.0;
    } else if( old_state <= params[1] ) {
        // Any dead cell with exactly three live neighbours becomes
        //  a live cell, as if by reproduction.
       if( (count > 2.5*(1.0+old_state)) && (count < 3.5*(1.0+old_state)) )
           new_state = 1.0;
    } else if( old_state <= params[2] ) {
        // Any dead cell with exactly three live neighbours becomes
        //  a live cell, as if by reproduction.
       if( (count > 2.5*(1.0+old_state)) && (count < 3.5*(1.0+old_state)) )
           new_state = 1.0;
    } else if( old_state <= params[3] ) {
        // Any dead cell with exactly three live neighbours becomes
        //  a live cell, as if by reproduction.
       if( (count > 2.5*(1.0+old_state)) && (count < 3.5*(1.0+old_state)) )
           new_state = 1.0;
...
def on_character(character):
    if character == "S":
        ...
        compute["texture"] = Z
    if character == "A":
        params = np.random.rand(4).astype(np.float32).sort()
        ...
        print(f'Parameters mutated to: {params}')
...
compute['dy'] = 1.0 / h
compute['pingpong'] = pingpong
compute['params'] = np.array([0.25, 0.5, 0.75, 1.0], dtype=np.float32)
